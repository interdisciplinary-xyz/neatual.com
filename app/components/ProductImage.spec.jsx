...
describe("ProductImage", () => {
  it("renders a WebP source ahead of a JPEG fallback", () => {
    const { container } = render(
      <ProductImage base={BASE} alt="Uniform" width={400} height={400} sizes="120px" />
    );

    const source = container.querySelector("source");
    expect(source).toHaveAttribute("type", "image/webp");
    expect(source.getAttribute("srcset")).toContain(`${BASE}-${IMAGE_WIDTHS[0]}.webp`);
  ...
  it("always carries intrinsic dimensions and async decoding", () => {
    render(<ProductImage base={BASE} alt="Uniform" width={400} height={400} sizes="120px" />);
    const img = screen.getByRole("img");
  ...
  it("has no axe violations, and an empty alt stays decorative", async () => {
    const { container } = render(
      <>
        <ProductImage base={BASE} alt="Uniform wzór nr 2" width={400} height={400} sizes="120px" />
